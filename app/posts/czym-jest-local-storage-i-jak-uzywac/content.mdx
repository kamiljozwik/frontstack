...
Choć Local Storage jest bardzo użytecznym narzędziem, istnieją też inne technologie przechowywania danych na stronie klienta, które mogą lepiej pasować do niektórych zastosowań. Są to między innymi cookies i IndexedDB. Cookies są idealne do przechowywania małych ilości danych i są jedyną technologią, która pozwala na wysyłanie danych do serwera z każdym żądaniem. IndexedDB natomiast jest bazą danych działającą w przeglądarce, która umożliwia przechowywanie dużych ilości strukturyzowanych danych i wykonanie na nich operacji w sposób asynchroniczny. O tych technologiach będziemy na pewno rozmawiać w kolejnych wpisach 🙂
