...
We wspomnianym już wcześniej przypadku użycia na mojej [stronie TIL](https://www.frontstack.pl/til), liczniki CSS stanowią jedynie dekorację dla tytułu pojedynczej sekcji, więc nie mają negatywnego wpływu na dostępność tej strony. Podobnie jest z numerowaniem linijek w kodzie źródłowym, gdzie liczniki CSS ponownie są tylko dekoracją.
