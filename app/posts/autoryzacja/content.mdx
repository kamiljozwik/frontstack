...
Z racji tego, że musiałem w ostatnim czasie odświeżyć swoją wiedzę dotyczącą autoryzacji, postanowiłem, że jest to dobry moment, aby podzielić się swoją wiedzą na ten temat.
...
Zrozumienie protokołów uwierzytelniania, takich jak OAuth, OpenID Connect i SAML, jest niezwykle ważne dla zaprojektowania bezpiecznego i wydajnego systemu uwierzytelniania. Każdy z tych protokołów oferuje różne mechanizmy kontroli dostępu, które mogą być dostosowane do specyficznych potrzeb aplikacji. Ponadto, zasady zapewnienia bezpiecznego uwierzytelniania - poufność, integralność i dostępność - muszą być ściśle przestrzegane, aby zminimalizować ryzyko naruszeń bezpieczeństwa.
...
Tokeny (np. `JWT` ([_JSON Web Tokens_](https://jwt.io/))) i sesje są dwoma dominującymi metodami zarządzania stanem uwierzytelnienia w aplikacjach webowych.
...
[OpenID Connect](https://openid.net/developers/how-connect-works/) (OIDC) działa na bazie OAuth 2.0 i dodaje warstwę uwierzytelniania do procesu autoryzacji. Pozwala klientom na weryfikację tożsamości użytkownika końcowego na podstawie uwierzytelnienia przeprowadzonego przez serwer autoryzacji.
...
OpenID Connect, zbudowany na fundamencie OAuth 2.0, dodaje warstwę uwierzytelniania do procesu autoryzacji. OIDC umożliwia klientom (aplikacjom) weryfikację tożsamości użytkownika końcowego oraz uzyskanie podstawowych informacji o profilu użytkownika na podstawie uwierzytelnienia przeprowadzonego przez serwer autoryzacji. OIDC wprowadza pojęcie _ID tokenu_, który zawiera informacje o tożsamości użytkownika oraz jest podpisany przez serwer autoryzacji, co umożliwia jego weryfikację przez aplikację.
...
- **Prostota i intuicyjność**: Interfejsy uwierzytelniania powinny być proste i intuicyjne, aby maksymalnie ułatwić użytkownikom proces autoryzacji. Kompleksowe formularze i procesy mogą zniechęcać i prowadzić do błędów, które z kolei mogą wpływać na bezpieczeństwo.
...
- **Federated SSO**: Użytkownik autoryzuje się za pośrednictwem swojego domowego IdP, który następnie udziela autoryzacji dla różnych aplikacji, nawet jeśli są one hostowane przez różne organizacje.
...
Magic linki możecie zobaczyć w praktyce na mojej [stronie z kursami](https://kursy.frontstack.pl/login). Zdecydowałem się na skorzystanie z tej opcji właśnie ze względu na wygodę użytkowników. Chociaż muszę przyznać, że zdarzały mi się sytuacje, gdzie użytkownicy nie do końca rozumieli, jak to działa i czemu nie mogą po prostu wpisać swojego hasła 😉
...
Autoryzacja jest kluczowym elementem większości aplikacji, dlatego też bardzo często będziemy korzystać z gotowych, sprawdzonych w boju rozwiązań.
